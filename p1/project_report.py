...
class cpu_scheduling:
    ...
    * Wait time - time spent in the ready queue
    ...
    def __init__(self, kwargs):
        ...
        self.process_cpuburstTime = dict(
            zip(self.process_queue, [[] for i in range(self.n)]))  # these are not stored in the pcb
    ...
    def msg(self, pid, info):
        ...
        # set a variable to control RR "time slice expire; ...continue" output once/per burst
            ...
            print("time %sms: Process %s switching out of CPU; will block on I/O until time %sms [Q " %
                  (self.system_clock, pid,
                   self.system_clock + self.mini_pcb[pid]["current_io_burst_t"] + int(self.tcs / 2)), end='')
            print_ready_queue()
            ...
            total_cxtswitch_no = sum(
                [self.mini_pcb[pid]["total_cxtswitch_no"] for pid in self.process_queue]) / 2  # in + out
            ...
            print('''%s,'''
                  ...
                  '''%d,'''
                  '''%d\n''' %
                  (self.alpha, self.rr_add, self.t_slice,
                   self.algo, avg_cpu_burst, avg_wait_time, avg_turnaround_time, total_cxtswitch_no,
                   total_preemption_no), end=''
                  , file=open("./simout_%s_%s.txt" % (self.bound_type, self.tcs), 'a+'))
        else:
    ...
    def ready_queue_add(self, pid):
        ...
        if self.algo == "RR":
            ...
            self.ready_queue.append(pid)
            l = zip(self.ready_queue,
                    [self.mini_pcb[pid]["remain_current_cpu_burst_t_est"] for pid in self.ready_queue])
            l = sorted(l, key=lambda t: t[0])  # first sort by pid 'A', 'B', ... # implicit tie handler
    ...
    def compare(self, running, pid2, data):
        ...
        if data == "remain_job_time":
            if self.mini_pcb[running]["remain_current_cpu_burst_t_est"] > self.mini_pcb[pid2][
                "remain_current_cpu_burst_t_est"]:
    ...
    def pcb_update(self, kwargs):
        ...
        if kwargs["event"] == 1:  # a new process has arrived, send to ready queue
            for pid in sorted(kwargs['pid']):  # add alphabetically to the ready queue
                self.pcb_add_job(pid)
                self.send_to_ready_queue(pid)
                self.msg(pid, "proc-arrive")
    ...
    def cpu_simulator(self, algo, bound_type):
        '''
        ...
        def get_bursts():
            '''
            For report, 20 processes * 50 bursts

            half CPU-bound processes and half IO-bound processes
            ...
            if self.bound_type == "cpu":
                ...
                for pid in self.process_queue:
                    self.mini_pcb[pid]["cpu_burst_no"] = 50

                    for i in range(49):
                        self.process_cpuburstTime[pid].append(100)
                        self.process_ioburstTime[pid].append(10)
                    self.process_cpuburstTime[pid].append(100)

            elif self.bound_type == "io":
                ...
                for pid in self.process_queue:
                    ...
                    for i in range(49):
                        self.process_cpuburstTime[pid].append(10)
                        self.process_ioburstTime[pid].append(100)
                    self.process_cpuburstTime[pid].append(10)
                ...
                for pid in self.process_queue[:int(len(self.process_queue) / 2)]:
                    self.mini_pcb[pid]["cpu_burst_no"] = 50
        ...
        while (1):  #
            if i < len(self.process_queue):
                ...
                if next_job_in_sec == 0:
                    job_arrive_l = []
                    # get all the job arrivals in this sec
                    while (arrival_l[i][0] - self.system_clock) == 0:
                        job_arrive_l.append(job_coming.__next__())
                        i += 1
                        if i >= len(self.process_queue):
                            break
                    self.pcb_update({'event': 1, 'pid': job_arrive_l})
                else:
...
if __name__ == "__main__":
    ...
    if debugging:
        sys.argv = ["project_report.py", 2, 0.01, 200, 20, 2, 0.1, 10, "END"]
    ...
    for algo in algo_list:
        scheduler.cpu_simulator(algo, bound_type)
