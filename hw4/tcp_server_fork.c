...
int main()
{
  ...
  while ( 1 )
  {
    /* TO DO: check for zombie processes here using waitpid( ... WNOHANG ) */

    printf( "SERVER: Blocked on accept()\n" );
    int newsd = accept( sd, (struct sockaddr *)&client, (socklen_t *)&fromlen );  // select / accept? - newsd ?
    ...
    /* hand the connection off to a child process */
    pid_t pid = fork();
    ...
    if ( pid > 0 )   /* PARENT */
    {
      ...
      /* PARENT currently ignores zombie processes ... */
    }
    else  /* CHILD */
    {
      do
      ...
      return EXIT_SUCCESS;  /* be sure to exit the child process */
